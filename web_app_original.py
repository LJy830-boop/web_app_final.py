...
# 使用markdown避免字符显示问题
st.markdown("# 电池健康状态(SOH)和剩余使用寿命(RUL)预测系统 - 原始版本")
...
# 预测函数 - 原始版本（SOH低于80%时RUL为0）
def predict_battery(df_cycle):
    """预测电池SOH和RUL - 原始版本，SOH低于80%时RUL为0"""
    try:
        # 提取最后一个循环的放电容量
        ...
        if len(df_cycle) > 1:
            ...
            # 如果只有一个循环数据，使用默认衰减率
            avg_decline_per_cycle = 0.2  # 假设每个循环SOH下降0.2%
        
        # 原始RUL计算逻辑：SOH低于80%时RUL为0
        if soh > 80:
            # 计算达到80%SOH还需要的循环次数
